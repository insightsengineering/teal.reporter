...
Reporter Previewer is a shiny module for visualize and manipulate the already added report Cards, and eventually downloading a report document.
...
1. Create a `tabsetPanel` with Main app add the Previewer.
      ...
      card$append_text(
        paste(
          c(
            'lyt <- basic_table() %>%',
            'split_rows_by("Month", label_pos = "visible") %>%',
            paste0('analyze("Ozone", afun = ', input$stat, ')'),
            'build_table(lyt, airquality)'
          ),
          collapse = "\n"
        ), "verbatim")
