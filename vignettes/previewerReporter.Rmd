      ...
      sidebarLayout(
        ...
        mainPanel(
          ...
          tabsetPanel(
            ...
            tabPanel("Table_2", verbatimTextOutput("table2")),
            tabPanel("Table_3", dataTableOutput("table3"))
          )
        )
      )
    ),
...
server <- function(input, output, session) {
  output$encoding <- renderUI({
    ...
    } else if (input$tabs %in% c("Table", "Table_2", "Table_3")) {
      sidebarPanel(
        selectInput(
          "stat",
          label = "Statistic",
  ...
  card_fun <- function(card = ReportCard$new(), comment) {
    if (input$tabs == "Plot") {
      ...
      paste(
        c(
          'aggregate(airquality[, c("Ozone"), drop = FALSE], list(Month = airquality$Month),', 
          paste0(input$stat, ","), 
          'na.rm = TRUE)'
        ),
        collapse = "\n"
      ), "verbatim")
    } else if (input$tabs == "Table_3") {
      card$set_name("Table Module 3")
      card$append_text("My Table 3", "header2")
      card$append_table(table3())
      card$append_text(
      paste(
        c(
          'aggregate(airquality[, c("Ozone"), drop = FALSE], list(Month = airquality$Month),', 
