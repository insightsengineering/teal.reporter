      ...
      sidebarLayout(
        ...
        mainPanel(
          ...
          tabsetPanel(
            ...
            tabPanel("Table", shiny::verbatimTextOutput("table", placeholder = TRUE))
          )
        )
      )
    ),
...
server <- function(input, output, session) {
  ...
  table <- reactive({
    ...
    if (input$formats == "data.frame") {
      tab <- aggregate(airquality[, c("Ozone"), drop = FALSE], list(Month = airquality$Month), mean, na.rm = TRUE)
      row.names(tab) <- NULL
      ...
      tab <- build_table(lyt, airquality)
      print(tab)
    }
  })
